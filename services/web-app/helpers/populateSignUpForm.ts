  ...
  try {
    ...
    if (data.data.user.__typename === "QueryUserSuccess" && data.data.user.data.username) {
      return {
        ...
        description: data.data.user.data.description ? data.data.user.data.description : '',
        contracts: data.data.user.data.contracts.map((contract) => contract.contractAddress),
      }
    }
    return {
      userExists: false,
